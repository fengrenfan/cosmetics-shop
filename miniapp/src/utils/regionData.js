...
export function getRegionColumns() {
  ...
  const districts = regionData[0]?.children?.[0]?.children?.map(d => ({ label: d.label, value: d.value })) || [];
  return [provinces, cities, districts];
}

export function getCities(provinceIndex) {
  const province = regionData[provinceIndex];
  if (!province) return [];
  return province.children.map(c => ({ label: c.label, value: c.value }));
}

export function getDistricts(provinceIndex, cityIndex) {
  const province = regionData[provinceIndex];
  if (!province) return [];
